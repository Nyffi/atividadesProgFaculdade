// Sintese
//   objetivo "LDE - Lista Duplamente Encadeada com CRUD e Descritor"
...
void infoAtual (struct tDescritor);
...
void infoAtual (struct tDescritor lst) {
    struct tNo *p = lst.ini;
    int qnt = lst.qtd, qntVog, qntCons, qntInv, i;
    float pctgVog, pctgCons, pctgInv;
    char primLetra;

    qntVog = qntCons = qntInv = 0;
    pctgVog = pctgCons = pctgInv = 0.0;

    if (qnt > 0) {
        for (i = 0; i < qnt; i++) {
            primLetra = toupper(p->dado.descricao[0]);

            if (isalpha(primLetra)) {
                switch (primLetra) {
                    case 'A':
                    case 'E':
                    case 'I':
    ...
    pctgVog = (float)qntVog / qnt * 100;
    pctgCons = (float)qntCons / qnt * 100;
    pctgInv = (float)qntInv / qnt * 100;

    printf("Quantidade de nos: %d\n\n", qnt);
    printf("Porcentagem de Vogais: %.2f%% \nPorcentagem de Consoantes: %.2f%% \nPorcentagem de Caracteres Invalidos: %.2f%% \n", pctgVog, pctgCons, pctgInv);
}
//*** FIM **********************************************************************
